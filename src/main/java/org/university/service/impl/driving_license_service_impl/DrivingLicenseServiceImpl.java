    ...
    public class DrivingLicenseServiceImpl implements DrivingLicenseService {
        ...
        public void assignDrivingLicenseToEmployee(Long licenseId, Long employeeId) throws DAOException {
            ...
            if(employee.getDrivingLicense() != null
                ...
                drivingLicenseDao.updateDrivingLicense(oldLicense.getId(), oldLicense);
            }
            ...
            drivingLicenseDao.updateDrivingLicense(licenseId, drivingLicense);
        }
        ...
        public Long getEmployeeIdForDrivingLicense(Long licenseId) throws DAOException {
            DrivingLicense dl = drivingLicenseDao.getDrivingLicenseById(licenseId);
            if (dl == null) throw new DAOException("DrivingLicense with id " + licenseId + " does not exist");

            return employeeDao.getAllEmployees().stream()
                    .filter(e -> e.getDrivingLicense() != null && licenseId.equals(e.getDrivingLicense().getId()))
                    .map(Employee::getId)
                    .findFirst()
                    .orElseThrow(() -> new DAOException("DrivingLicense with id " + licenseId + " is not assigned to any employee"));
        }
    }
